...
from evaluation.eval import evaluate_all_retrieval, evaluate_all_answers, MODEL as JUDGE_MODEL
from answer import MODEL as RAG_MODEL, EMBEDDING_MODEL_NAME
...
def main():
    ...
    with gr.Blocks(title="RAG Evaluation Dashboard", theme=theme) as app:
        ...
        # Models in use
        gr.Markdown("## ⚙️ Models in use")
        models_html = f"""
        <div style="padding: 0;">
            <div style="margin: 10px 0; padding: 15px; background-color: #f5f5f5; border-radius: 8px; border-left: 5px solid #4a90d9;">
                <div style="font-size: 14px; color: #666; margin-bottom: 5px;">Embedding Model</div>
                <div style="font-size: 20px; font-weight: bold; color: #4a90d9;">{EMBEDDING_MODEL_NAME}</div>
            </div>
            <div style="margin: 10px 0; padding: 15px; background-color: #f5f5f5; border-radius: 8px; border-left: 5px solid #4a90d9;">
                <div style="font-size: 14px; color: #666; margin-bottom: 5px;">RAG LLM</div>
                <div style="font-size: 20px; font-weight: bold; color: #4a90d9;">{RAG_MODEL}</div>
            </div>
            <div style="margin: 10px 0; padding: 15px; background-color: #f5f5f5; border-radius: 8px; border-left: 5px solid #4a90d9;">
                <div style="font-size: 14px; color: #666; margin-bottom: 5px;">Judge LLM</div>
                <div style="font-size: 20px; font-weight: bold; color: #4a90d9;">{JUDGE_MODEL}</div>
            </div>
        </div>
        """
        gr.HTML(models_html)
